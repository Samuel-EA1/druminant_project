...
export default function Livestock() {
  ...
  const handleEdit = (tagId) => {
    ...
    const selectedRecord = // Logic to fetch the record based on `id`
      setEditFormInput({
        ...
        emailAdd: selectedRecord.emailAdd,
      });
    setEditId(tagId);
  ...
  const handleFormSubmit = (e) => {
    ...
    if (isTagIdUsed) {
      alert(
        `Tag ID '${tagId}' is already used. Please choose a different Tag ID.`
      );
      return;
    }
    const entryRecord = {
    ...
    const updatedLivestockData = [
      entryRecord,
      ...livestockData.map((record) => ({
        ...record,
        id: record.id + 1,
      })),
    ];
    ...
    <div className="livestock">
      <div className="relative">
        ...
        <div>
          <div className="up">
            <div>
              <h1 className="module-header">Staff Requests</h1>
              <p>No. of Staff Request (5)</p>
            </div>
          </div>
        </div>
          ...
          <thead>
            <tr>
              <th
                ...
                style={{ backgroundColor: "rgb(7, 78, 0)" }}
              >
                ID
              </th>
              <th
              ...
              >
                <td className="w-full md:w-auto flex justify-between items-center p-3 text-gray-800 text-center border border-b text-center block md:table-cell relative md:static">
                  <span
                    ...
                    style={{ backgroundColor: "#c1ffb4", fontSize: "11px" }}
                  >
                    username
                  </span>
                  <div style={{ fontSize: "14px" }}>{row.id}</div>
                </td>
...

        {
          ...
          formModal && (
            <div
              ...
              class=" py-12 bg-white transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0"
              id="modal"
            >
